...
function plugin(settings, options) {
  return function(Model) {
    ...
    Model
      .before('findOne', store.findOne)
      .before('findAll', store.findAll)
      .before('count', store.count)
      .before('removeAll', store.removeAll)
      .before('save', store.save)
      .before('remove', store.remove);
  };
};
...
store.findOne = function(id, callback) {
  var Model = this;
 ...
 *
 * @param {Object} query
 * @param {Number} maxLimit maximum value for query.limit
 * @return {Array}
 * @api private
 */

store.collection = function(query, maxLimit) {
...
store.findAll = function(query, callback) {
  var pool = this.options.mysql.pool;
  var collection = store.collection(query, this.options.maxLimit);
  var Model = this;
  ...
  async.waterfall([
    ...
    function(conn, next) {
      ...
      store.query(Model, sql.query, sql.values, function(err, rows, fields, conn) {
        ...
        for (var i=0; i<collection.limit; i++) {
          if (stripTableName(rows[collection.offset + i], Model).id) {
            ids.push(stripTableName(rows[collection.offset + i], Model).id);
          }
        }
        next(null, ids, conn);
      }, conn);
...
/**
 * Remove all models with given `query`.
 *
 * @param {Object} query
...
store.removeAll = function(query, callback) {
  var tableName = this.options.mysql.tableName;
  var primaryKey = this.primaryKey;

  var sql = store.queryToSQL(this, merge(merge({
    type: 'delete',
    groupBy: [ tableName + '.' + primaryKey ],
    columns: [
      { name: primaryKey, table: tableName }
    ],
    table: tableName,
  }, query), { offset: null, limit: null }));
...
store.save = function(model, changed, done) {
  var Model = this;

  if (model.primary && !changed[Model.primaryKey]) {
    return store.update.call(Model, model, changed, done);
  }
...
store.update = function(model, changed, done) {
  var Model = this;

  var where = {};
  where[Model.primaryKey] = model.primary;
...
store.remove = function(model, done) {
  var Model = this;
  ...
  query.where[Model.primaryKey] = model.primary;
