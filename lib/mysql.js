  ...
  async.waterfall([
    ...
    function(conn, next) {
      var sql = Model.adapter.queryToSQL(merge(merge({
        type: 'select',
        groupBy: [ Model.options.tableName + '.' + Model.primaryKey ],
        columns: [
          { name: Model.primaryKey, table: Model.options.tableName }
        ],
        table: Model.options.tableName,
      }, query), { offset: null, limit: null, include: null }));
      Model.adapter.query(sql.query, sql.values, function(err, rows, fields, conn) {
        if (err) return next(err);
        var ids = [];
    ...
    function(ids, conn, next) {
      ...
      if (query.include) idQuery.include = query.include;
      var sql = Model.adapter.queryToSQL(idQuery);
      Model.adapter.query(sql.query, sql.values, function(err, rows, fields, conn) {
        if (err) return next(err);
        ...
        Model.relations.forEach(function(relation) {
          ...
          var foreignModel = relation.type === 'has one' ? Model : relation.anotherModel;
          var foreignKey = (relation.through || relation.anotherModel).options.tableName + '_foreign_key';
          var related_collections = collections[relation.as] = {};
          for (var len = rows.length, i=0; i<len; i++) {
            var id = rows[i][Model.options.tableName + '_' + Model.primaryKey];
            if (!related_collections[id]) {
              var model = stripTableName(rows[i], Model);
              model.related = {};
              collection.push(model);
              related_collections[id] = relation.type === 'has one' ? null : [];
            }
            if (rows[i][foreignKey]) {
              ...
              if (relation.type === 'has one') {
                related_collections[id] = related;
              }
              else {
                related_collections[id].push(related);
              }
            }
          }
          for (var len = collection.length, i=0; i<len; i++) {
            var related_collection = related_collections[collection[i][Model.primaryKey]];
            if (related_collection) {
              if (relation.type === 'has one') {
                collection[i].related[relation.as] = new relation.anotherModel(related_collection);
              }
              else {
                collection[i].related[relation.as] = hydrate(related_collection, relation.anotherModel);
              }
            }
            else {
              collection[i].related[relation.as] = relation.type === 'has one' ? null : [];
            }
          }
        });
        next(null, conn);
    ...
    if (query.include) {
      ...
      for (var len = this.relations.length, i=0; i<len; i++) {
        ...
        if (~query.include.indexOf(relation.as) && relation.model == this) {
          model.related[relation.as] = model.related[relation.as] || (relation.type === 'has one' ? null : []);
          for (var len = rows.length, i=0; i<len; i++) {
            ...
            if (rows[i][foreignKey]) {
              ...
              if (relation.type === 'has one') {
                model.related[relation.as] = new relation.anotherModel(related);
              }
              else {
          ...
          hydrate(model.related[relation.as], relation.anotherModel);
        }
      }
    }
...
adapter.query = function(statement, values, callback, existingConn) {
  var Model = this;
  var pool = Model.adapter.settings.pool;
  ...
  function query(err, conn) {
    ...
    function done(err, rows, fields) {
      ...
      debug("%s", statement.sql || statement);
  ...
  if (typeof attrs.related === 'object') {
    ...
    for (var as in attrs.related) {
      ...
      for (var len = relations.length, i=0; i<len; i++) {
        ...
        if (params.as == as) {
          if (attrs.related[as] instanceof Array) {
            hydrate(attrs.related[as], relations[i].anotherModel);
            model.related[as] = attrs.related[as];
          }
          else {
            model.related[as] = new relations[i].anotherModel(attrs.related[as]);
          }
        }
      }
    }
